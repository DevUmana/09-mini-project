...
const getParksByState = async (state: string): Promise<Park[]> => {
  try {
    ...
    const parks = await fetch(`/api/parks/${state}`);

    if (!parks.ok) {
      throw new Error("Failed to fetch parks by state");
    }

    const parksData: Park[] = await parks.json();
    return parksData;
  } catch (err) {
...
const deledStateFromHistory = async (id: string) => {
  ...
  try {
    const stateFromHistory = await fetch(`/api/history/${id}`, {
      method: "DELETE",
      headers: {
    ...
    if (!stateFromHistory.ok) {
      throw new Error("Failed to delete state from history");
    }
...
const getEventsByState = async (state: string): Promise<ParkEvent> => {
  ...
  try {
    const events = await fetch(`/api/parks/events/${state}`);

    if (!events.ok) {
      throw new Error("Failed to fetch events by state");
    }

    const eventsData: ParkEvent = await events.json();
    return eventsData;
  } catch (err) {
    console.log("Error:", err);
    return err as ParkEvent;
  }
};
// * Function to get saved searches

const getHistory = async (): Promise<State[]> => {
  ...
  try {
    ...
    const historyData: State[] = await history.json();
    return historyData;
  } catch (err) {
    console.log("Error:", err);
    // Is this correct?
